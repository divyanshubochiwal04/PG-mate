...
exports.KyselyMessReportingRepository = void 0;
const kysely_1 = require("kysely");
const contracts_1 = require("@m-square/contracts");
class KyselyMessReportingRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    async getMessReport(organizationId, filter) {
        ...
        const countRes = await baseQuery
            .select((0, kysely_1.sql) `COUNT(DISTINCT ms.id)::int`.as('total'))
            .executeTakeFirstOrThrow();
        const total = countRes.total || 0;
        const summaryRes = await this.db
            .selectFrom('resident_mess_subscriptions as ms')
            .select([
            (0, kysely_1.sql) `COUNT(CASE WHEN ms.status = 'ACTIVE' THEN 1 END)::int`.as('activeSubs'),
            (0, kysely_1.sql) `COUNT(CASE WHEN ms.status = 'CANCELLED' THEN 1 END)::int`.as('cancelledSubs'),
            (0, kysely_1.sql) `COALESCE(SUM(CASE WHEN ms.status = 'ACTIVE' THEN ms.price_at_subscription ELSE 0 END), 0)::float`.as('monthlyValue'),
        ])
            .where('ms.organization_id', '=', organizationId)
            .executeTakeFirst();
        const consRes = await this.db
            .selectFrom('mess_meal_consumptions')
            .select((0, kysely_1.sql) `COUNT(id)::int`.as('totalConsumptions'))
        ...
        const rows = await baseQuery
            ...
            (0, kysely_1.sql) `ms.price_at_subscription::float`.as('monthlyPrice'),
...
exports.KyselyMessReportingRepository = KyselyMessReportingRepository;
//# sourceMappingURL=reporting-mess.repository.js.map