...
import type {
  ActivityQueryResultItem,
  BillingQueryResult,
  ExpenseCategoryQueryResult,
  ExpenseQueryResult,
  MessQueryResult,
  OccupancyQueryResult,
  OperationalAlertQueryResultItem,
  ResidentQueryResult,
} from './reporting-query-types';

export * from './reporting-query-types';

export class KyselyReportingRepository {
  constructor(private readonly db: Kysely<DatabaseSchema>) {}

  public async getOccupancyMetrics(
    organizationId: string,
    propertyId?: string,
    buildingId?: string
  ): Promise<OccupancyQueryResult> {
    let propQuery = this.db
      .selectFrom('properties')
      .select(sql<number>`COUNT(id)::int`.as('count'))
      .where('organization_id', '=', organizationId);
    if (propertyId) propQuery = propQuery.where('id', '=', propertyId);
    ...
    let bldgQuery = this.db
      ...
      .select(sql<number>`COUNT(id)::int`.as('count'))
      .where('organization_id', '=', organizationId);
    if (propertyId) bldgQuery = bldgQuery.where('property_id', '=', propertyId);
    if (buildingId) bldgQuery = bldgQuery.where('id', '=', buildingId);
    ...
    let flrQuery = this.db
      ...
      .select(sql<number>`COUNT(f.id)::int`.as('count'))
      .where('f.organization_id', '=', organizationId);
    if (propertyId) flrQuery = flrQuery.where('b.property_id', '=', propertyId);
    if (buildingId) flrQuery = flrQuery.where('b.id', '=', buildingId);
    ...
    let rmQuery = this.db
      ...
      .innerJoin('buildings as b', 'b.id', 'f.building_id')
      .select(sql<number>`COUNT(r.id)::int`.as('count'))
      .where('r.organization_id', '=', organizationId);
    if (propertyId) rmQuery = rmQuery.where('b.property_id', '=', propertyId);
    if (buildingId) rmQuery = rmQuery.where('b.id', '=', buildingId);
    ...
    if (propertyId) bedsBaseQuery = bedsBaseQuery.where('b.property_id', '=', propertyId);
    if (buildingId) bedsBaseQuery = bedsBaseQuery.where('b.id', '=', buildingId);

    const bedsStatusRes = await bedsBaseQuery
      .select(['bd.status as status', sql<number>`COUNT(bd.id)::int`.as('count')])
      .groupBy('bd.status')
      .execute();
    ...
    for (const r of bedsStatusRes) {
      ...
      totalBeds += cnt;
      if (r.status === 'MAINTENANCE') maintenanceBeds += cnt;
      if (r.status === 'INACTIVE') inactiveBeds += cnt;
    }
    ...
    let activeAllocQuery = this.db
      ...
      .innerJoin('floors as f', 'f.id', 'r.floor_id')
      .innerJoin('buildings as b', 'b.id', 'f.building_id')
      .select(sql<number>`COUNT(DISTINCT ba.bed_id)::int`.as('count'))
      .where('ba.organization_id', '=', organizationId)
      .where('ba.status', '=', 'ACTIVE');
    if (propertyId) activeAllocQuery = activeAllocQuery.where('b.property_id', '=', propertyId);
    if (buildingId) activeAllocQuery = activeAllocQuery.where('b.id', '=', buildingId);
  ...
  public async getResidentMetrics(
    organizationId: string,
    propertyId?: string,
    buildingId?: string,
    startDate?: string,
    endDate?: string
  ): Promise<ResidentQueryResult> {
    const resStatusQuery = this.db
      .selectFrom('residents')
      .select(['status', sql<number>`COUNT(id)::int`.as('count')])
      .where('organization_id', '=', organizationId)
      .groupBy('status');
    ...
    for (const r of resStatus) {
      if (r.status === 'ACTIVE') totalActiveResidents = Number(r.count);
      if (r.status === 'INACTIVE') totalInactiveResidents = Number(r.count);
    }
    ...
    if (propertyId || buildingId) {
      staysBase = staysBase
        ...
        .innerJoin('floors as f', 'f.id', 'r.floor_id')
        .innerJoin('buildings as b', 'b.id', 'f.building_id');
      if (propertyId) staysBase = staysBase.where('b.property_id', '=', propertyId);
      if (buildingId) staysBase = staysBase.where('b.id', '=', buildingId);
    }

    const currentCheckedInRes = await staysBase
      .select(sql<number>`COUNT(DISTINCT s.resident_id)::int`.as('count'))
      .where('s.status', '=', 'ACTIVE')
      .executeTakeFirst();
    ...
    let admissionsQuery = staysBase.select(sql<number>`COUNT(DISTINCT s.id)::int`.as('count'));
    if (startDate)
    ...
    let checkoutsQuery = staysBase
      .select(sql<number>`COUNT(DISTINCT s.id)::int`.as('count'))
      .where('s.status', 'in', ['COMPLETED', 'CHECKED_OUT']);
    if (startDate)
    ...
    let transfersQuery = this.db
      .selectFrom('bed_allocations as ba')
      .select(sql<number>`COUNT(ba.id)::int`.as('count'))
      .where('ba.organization_id', '=', organizationId)
      .where('ba.status', '=', 'TRANSFERRED');
    if (startDate)
      transfersQuery = transfersQuery.where('ba.created_at', '>=', new Date(startDate));
    if (endDate) transfersQuery = transfersQuery.where('ba.created_at', '<=', new Date(endDate));
  ...
  public async getBillingMetrics(
    ...
    if (startDate) invQuery = invQuery.where('issued_at', '>=', new Date(startDate));
    if (endDate) invQuery = invQuery.where('issued_at', '<=', new Date(endDate));

    const invSummary = await invQuery
      .select([
        sql<number>`COALESCE(SUM(ROUND(total_amount * 100)), 0)::bigint`.as('totalInvoiced'),
        sql<number>`COALESCE(SUM(ROUND(paid_amount * 100)), 0)::bigint`.as('totalCollected'),
        sql<number>`COALESCE(SUM(ROUND(balance_due_amount * 100)), 0)::bigint`.as(
          'totalOutstanding'
        ),
        sql<number>`COALESCE(SUM(CASE WHEN status = 'OVERDUE' THEN ROUND(balance_due_amount * 100) ELSE 0 END), 0)::bigint`.as(
          'overdueAmount'
        ),
        sql<number>`COALESCE(SUM(CASE WHEN status = 'PARTIALLY_PAID' THEN ROUND(balance_due_amount * 100) ELSE 0 END), 0)::bigint`.as(
          'partiallyPaidAmount'
        ),
        sql<number>`COUNT(CASE WHEN status = 'PAID' THEN 1 END)::int`.as('paidCount'),
        sql<number>`COUNT(CASE WHEN status IN ('ISSUED', 'PARTIALLY_PAID', 'OVERDUE') THEN 1 END)::int`.as(
          'unpaidCount'
        ),
        sql<number>`COUNT(CASE WHEN status = 'OVERDUE' THEN 1 END)::int`.as('overdueCount'),
      ])
      .executeTakeFirst();
  ...
  public async getMessMetrics(
    organizationId: string,
    propertyId?: string,
    buildingId?: string,
    startDate?: string,
    endDate?: string
  ): Promise<MessQueryResult> {
    ...
    for (const m of mealsToday) {
      if (m.status === 'CONSUMED') mealsConsumedToday = Number(m.count);
      if (m.status === 'SKIPPED' || m.status === 'CANCELLED') mealsSkippedToday += Number(m.count);
    }
    ...
    const invRes = await this.db
      ...
      .select([
        sql<number>`COUNT(id)::int`.as('totalCount'),
        sql<number>`COALESCE(SUM(ROUND(current_stock * 0 * 100)), 0)::bigint`.as('totalValue'),
        sql<number>`COUNT(CASE WHEN current_stock <= reorder_level AND current_stock > 0 THEN 1 END)::int`.as(
          'lowStock'
        ),
        sql<number>`COUNT(CASE WHEN current_stock <= 0 THEN 1 END)::int`.as('outOfStock'),
      ])
      .where('organization_id', '=', organizationId)
      .executeTakeFirst();
    ...
    let procQuery = this.db
      ...
      .select(sql<number>`COALESCE(SUM(ROUND(total_amount * 100)), 0)::bigint`.as('total'))
      .where('organization_id', '=', organizationId);
    if (startDate) procQuery = procQuery.where('created_at', '>=', new Date(startDate));
    if (endDate) procQuery = procQuery.where('created_at', '<=', new Date(endDate));
    ...
    let expQuery = this.db
      ...
      .select(sql<number>`COALESCE(SUM(ROUND(amount * 100)), 0)::bigint`.as('total'))
      .where('organization_id', '=', organizationId);
    if (startDate) expQuery = expQuery.where('created_at', '>=', new Date(startDate));
    if (endDate) expQuery = expQuery.where('created_at', '<=', new Date(endDate));
  ...
  public async getExpenseMetrics(
    ...
    let messExpQuery = this.db
      ...
      .select(sql<number>`COALESCE(SUM(ROUND(amount * 100)), 0)::bigint`.as('total'))
      .where('organization_id', '=', organizationId);
    if (startDate) messExpQuery = messExpQuery.where('created_at', '>=', new Date(startDate));
    if (endDate) messExpQuery = messExpQuery.where('created_at', '<=', new Date(endDate));
    ...
    const categories: ExpenseCategoryQueryResult[] = [
  ...
  public async getRecentActivity(
    ...
    buildingId?: string,
    limit = 20
  ): Promise<ActivityQueryResultItem[]> {
    const items: ActivityQueryResultItem[] = [];
  ...
  public async getOperationalAlerts(
    ...
    propertyId?: string,
    buildingId?: string
  ): Promise<OperationalAlertQueryResultItem[]> {
    const alerts: OperationalAlertQueryResultItem[] = [];
