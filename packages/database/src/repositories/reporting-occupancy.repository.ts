...
import type { OccupancyReportResponseDto, OccupancyReportRowDto, ReportFilterDto } from '@m-square/contracts';

export class KyselyOccupancyReportingRepository {
  ...
  public async getOccupancyReport(
    ...
    let query = this.db
      ...
      .select([
        ...
        sql<number>`(SELECT COUNT(bd.id)::int FROM beds bd WHERE bd.room_id = rm.id AND bd.organization_id = ${organizationId})`.as('totalBeds'),
        sql<number>`(SELECT COUNT(DISTINCT ba.bed_id)::int FROM bed_allocations ba JOIN beds bd ON bd.id = ba.bed_id WHERE bd.room_id = rm.id AND ba.status = 'ACTIVE' AND ba.organization_id = ${organizationId})`.as('occupiedBeds'),
      ])
    ...
    const formattedRows: OccupancyReportRowDto[] = rows.map((r: any) => {
    ...
    const floorSet = new Set(rows.map((r: any) => r.floorId));
