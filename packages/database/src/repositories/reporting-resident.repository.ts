...
import type { ReportFilterDto, ResidentReportResponseDto, ResidentReportRowDto } from '@m-square/contracts';
import { calculatePaginationBounds } from '@m-square/contracts';

export class KyselyResidentReportingRepository {
  ...
  public async getResidentReport(
    ...
    const checkedOutRes = await this.db
      .selectFrom('stays')
      .select(sql<number>`COUNT(DISTINCT resident_id)::int`.as('cnt'))
      .where('organization_id', '=', organizationId)
    ...
    const noStayRes = await this.db
      ...
      .select(sql<number>`COUNT(r.id)::int`.as('cnt'))
    ...
    const occBedsRes = await this.db
      ...
      .select(sql<number>`COUNT(DISTINCT bed_id)::int`.as('cnt'))
      .where('organization_id', '=', organizationId)
      .where('status', '=', 'ACTIVE')
      .executeTakeFirst();
    ...
    const dueRes = await this.db
      .selectFrom('invoices')
      .select(sql<number>`COALESCE(SUM(balance_due_amount), 0)::float`.as('total'))
      .where('organization_id', '=', organizationId)
    ...
    let baseQuery = this.db
      ...
      .leftJoin('resident_mess_subscriptions as ms', (join) =>
        join.onRef('ms.stay_id', '=', 's.id').on('ms.status', '=', 'ACTIVE')
      )
      .where('r.organization_id', '=', organizationId);
    ...
    const countRes = await baseQuery
      .select(sql<number>`COUNT(DISTINCT r.id)::int`.as('total'))
    ...
    const rows = await baseQuery
      .select([
        ...
        sql<string>`CONCAT(r.first_name, ' ', r.last_name)`.as('fullName'),
        'r.phone as phone',
        'r.email as email',
        'r.status as status',
    ...
    const formattedRows: ResidentReportRowDto[] = rows.map((r: any) => ({
