...
exports.KyselyResidentOperationalRepository = void 0;
const kysely_1 = require("kysely");
const contracts_1 = require("@m-square/contracts");
class KyselyResidentOperationalRepository {
    db;
    constructor(db) {
        this.db = db;
    }
    async findOperationalList(organizationId, params) {
        ...
        let query = this.db
            ...
            .leftJoin('stays as s', (join) => join
            .onRef('s.resident_id', '=', 'r.id')
            .on('s.organization_id', '=', organizationId)
            .on((eb) => eb.or([
            eb('s.status', '=', 'ACTIVE'),
            eb('s.id', '=', (0, kysely_1.sql) `(SELECT s_sub.id FROM stays s_sub WHERE s_sub.resident_id = r.id AND s_sub.organization_id = ${organizationId} ORDER BY s_sub.created_at DESC LIMIT 1)`),
        ])))
            .leftJoin('bed_allocations as ba', (join) => join
            .onRef('ba.stay_id', '=', 's.id')
            .on('ba.organization_id', '=', organizationId)
            .on('ba.status', '=', 'ACTIVE'))
            .leftJoin('beds as b', (join) => join
            .onRef('b.id', '=', 'ba.bed_id')
            .on('b.organization_id', '=', organizationId))
            .leftJoin('rooms as rm', (join) => join
            .onRef('rm.id', '=', 'b.room_id')
            .on('rm.organization_id', '=', organizationId))
            .leftJoin('floors as fl', (join) => join
            .onRef('fl.id', '=', 'rm.floor_id')
            .on('fl.organization_id', '=', organizationId))
            .leftJoin('buildings as bg', (join) => join
            .onRef('bg.id', '=', 'fl.building_id')
            .on('bg.organization_id', '=', organizationId))
            .leftJoin('properties as pr', (join) => join
            .onRef('pr.id', '=', 'bg.property_id')
            .on('pr.organization_id', '=', organizationId))
            .leftJoin('resident_mess_subscriptions as ms', (join) => join
            .onRef('ms.stay_id', '=', 's.id')
            .on('ms.organization_id', '=', organizationId)
            .on('ms.status', '=', 'ACTIVE'))
            .leftJoin('mess_meal_plans as mp', (join) => join
        ...
        let countQuery = this.db
            ...
            .on('ms.organization_id', '=', organizationId)
            .on('ms.status', '=', 'ACTIVE'))
            .select((0, kysely_1.sql) `COUNT(DISTINCT r.id)`.as('total'));
        if (params.search && params.search.trim().length > 0) {
            const term = `%${params.search.trim().toLowerCase()}%`;
            const searchCond = (eb) => eb.or([
        ...
        if (params.messStatus && params.messStatus !== 'ALL') {
            if (params.messStatus === 'ACTIVE') {
                ...
                query = query.where((eb) => eb.or([eb('ms.id', 'is', null), eb('ms.status', '!=', 'ACTIVE')]));
                countQuery = countQuery.where((eb) => eb.or([eb('ms.id', 'is', null), eb('ms.status', '!=', 'ACTIVE')]));
            }
        }
        if (params.billingStatus && params.billingStatus !== 'ALL') {
            const dueSql = (0, kysely_1.sql) `COALESCE((
        SELECT SUM(inv.balance_due_amount)
        FROM invoices inv
        WHERE inv.resident_id = r.id
          AND inv.organization_id = ${organizationId}
        ...
        const rows = await query
            .orderBy((0, kysely_1.sql) `CASE WHEN s.status = 'ACTIVE' THEN 0 ELSE 1 END`, 'asc')
        ...
        const items = rows.map((row) => ({
            ...
            stayStatus: row.stayStatus ? row.stayStatus : null,
    ...
    async getOperationalSummary(organizationId) {
        ...
        const checkedOutRow = await this.db
            .selectFrom('residents as r')
            .select((0, kysely_1.sql) `COUNT(DISTINCT r.id)`.as('cnt'))
            .where('r.organization_id', '=', organizationId)
            .where((0, kysely_1.sql) `NOT EXISTS (SELECT 1 FROM stays s_act WHERE s_act.resident_id = r.id AND s_act.organization_id = ${organizationId} AND s_act.status = 'ACTIVE')`)
            .where((0, kysely_1.sql) `EXISTS (SELECT 1 FROM stays s_comp WHERE s_comp.resident_id = r.id AND s_comp.organization_id = ${organizationId} AND s_comp.status = 'COMPLETED')`)
            .executeTakeFirstOrThrow();
        const noStayRow = await this.db
            .selectFrom('residents as r')
            .select((0, kysely_1.sql) `COUNT(r.id)`.as('cnt'))
            .where('r.organization_id', '=', organizationId)
            .where((0, kysely_1.sql) `NOT EXISTS (SELECT 1 FROM stays s WHERE s.resident_id = r.id AND s.organization_id = ${organizationId})`)
            .executeTakeFirstOrThrow();
        ...
        const dueSumRow = await this.db
            .selectFrom('invoices')
            .select((0, kysely_1.sql) `COALESCE(SUM(balance_due_amount), 0)`.as('sum'))
            .where('organization_id', '=', organizationId)
...
exports.KyselyResidentOperationalRepository = KyselyResidentOperationalRepository;
//# sourceMappingURL=resident-operational.repository.js.map