...
async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('current_stock', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `0.00`))
        .addColumn('minimum_stock', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `0.00`))
        .addColumn('reorder_level', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `0.00`))
        .addColumn('status', 'varchar(20)', (col) => col.notNull().defaultTo('IN_STOCK'))
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addUniqueConstraint('uq_inv_items_id_org', ['id', 'organization_id'])
        .addForeignKeyConstraint('fk_inv_mess_org', ['mess_id', 'organization_id'], 'messes', ['id', 'organization_id'], (cb) => cb.onDelete('cascade'))
        .addCheckConstraint('check_inv_current_stock', (0, kysely_1.sql) `current_stock >= 0`)
        .addCheckConstraint('check_inv_minimum_stock', (0, kysely_1.sql) `minimum_stock >= 0`)
        .addCheckConstraint('check_inv_reorder_level', (0, kysely_1.sql) `reorder_level >= 0`)
    ...
    await db.schema
        ...
        .addColumn('quantity', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('stock_before', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('stock_after', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('unit', 'varchar(20)', (col) => col.notNull())
        .addColumn('procurement_id', 'uuid')
        .addColumn('notes', 'varchar(500)')
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addForeignKeyConstraint('fk_inv_tx_item_org', ['inventory_item_id', 'organization_id'], 'mess_inventory_items', ['id', 'organization_id'], (cb) => cb.onDelete('cascade'))
        .addCheckConstraint('check_inv_tx_quantity', (0, kysely_1.sql) `quantity > 0`)
        .addCheckConstraint('check_inv_tx_stock_before', (0, kysely_1.sql) `stock_before >= 0`)
        .addCheckConstraint('check_inv_tx_stock_after', (0, kysely_1.sql) `stock_after >= 0`)
        .addCheckConstraint('check_inv_tx_type', (0, kysely_1.sql) `transaction_type IN ('OPENING_STOCK', 'PURCHASE', 'ADJUSTMENT_IN', 'ADJUSTMENT_OUT', 'CONSUMPTION', 'WASTAGE')`)
        .execute();
    ...
    await db.schema
        ...
        .addCheckConstraint('check_proc_total_amount', (0, kysely_1.sql) `total_amount >= 0`)
        .execute();
    await db.schema
        ...
        .addCheckConstraint('check_proc_item_quantity', (0, kysely_1.sql) `quantity > 0`)
        .addCheckConstraint('check_proc_item_unit_price', (0, kysely_1.sql) `unit_price >= 0`)
        .addCheckConstraint('check_proc_item_total_price', (0, kysely_1.sql) `total_price >= 0`)
        .execute();
    ...
    await db.schema
        ...
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `NOW()`))
        .addForeignKeyConstraint('fk_exp_mess_org', ['mess_id', 'organization_id'], 'messes', ['id', 'organization_id'], (cb) => cb.onDelete('restrict'))
        .addCheckConstraint('check_expenses_amount', (0, kysely_1.sql) `amount > 0`)
        .addCheckConstraint('check_expenses_category', (0, kysely_1.sql) `category IN ('GAS', 'ELECTRICITY', 'SALARY', 'CLEANING', 'TRANSPORT', 'MAINTENANCE', 'MISCELLANEOUS')`)
        .execute();
...
//# sourceMappingURL=00007_mess_management_schema.js.map