...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addCheckConstraint('check_residents_status', sql`status IN ('ACTIVE', 'INACTIVE')`)
    .addCheckConstraint(
      'check_residents_gender',
      sql`gender IN ('MALE', 'FEMALE', 'OTHER', 'PREFER_NOT_TO_SAY')`
    )
    .execute();
  ...
  await db.schema
    ...
    .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`NOW()`))
    .addForeignKeyConstraint(
      'fk_emergency_contacts_resident_org',
    ...
    .addCheckConstraint(
      'check_emergency_contacts_relationship',
      sql`relationship IN ('PARENT', 'GUARDIAN', 'SIBLING', 'SPOUSE', 'FRIEND', 'OTHER')`
    )
    .execute();
  ...
  await db.schema
    ...
    .addColumn('organization_id', 'uuid', (col) => col.notNull())
    .addColumn('resident_id', 'uuid', (col) => col.notNull())
    .addColumn('admission_date', 'date', (col) => col.notNull().defaultTo(sql`CURRENT_DATE`))
    ...
    .addCheckConstraint('check_stays_status', sql`status IN ('ACTIVE', 'COMPLETED', 'CANCELLED')`)
    .addCheckConstraint(
      'check_stays_expected_checkout',
      sql`expected_checkout_date IS NULL OR expected_checkout_date >= admission_date`
    )
    .addCheckConstraint(
      'check_stays_actual_checkout',
      sql`actual_checkout_date IS NULL OR actual_checkout_date >= admission_date`
    )
    .addCheckConstraint(
      'check_stays_status_checkout_nullability',
      sql`(status = 'ACTIVE' AND actual_checkout_date IS NULL) OR (status IN ('COMPLETED', 'CANCELLED') AND actual_checkout_date IS NOT NULL)`
    )
    .execute();
  ...
  await db.schema
    ...
    .addColumn('start_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`NOW()`))
    .addColumn('end_at', 'timestamptz')
    .addColumn('status', 'varchar(20)', (col) => col.notNull().defaultTo('ACTIVE'))
    .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`NOW()`))
    .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`NOW()`))
    .addForeignKeyConstraint(
      'fk_allocations_stay_org',
    ...
    .addForeignKeyConstraint(
      'fk_allocations_bed_org',
      ['bed_id', 'organization_id'],
      'beds',
      ['id', 'organization_id'],
      (cb) => cb.onDelete('restrict')
    )
