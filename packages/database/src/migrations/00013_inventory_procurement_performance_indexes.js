...
async function down(db) {
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_uq_procurement_invoice_ref`.execute(db);
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_mess_exp_org_mess_date`.execute(db);
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_mess_proc_items_proc`.execute(db);
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_mess_proc_org_mess_date`.execute(db);
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_mess_vendors_org_status`.execute(db);
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_mess_inv_tx_org_item_date`.execute(db);
    await (0, kysely_1.sql) `DROP INDEX IF EXISTS idx_mess_inv_items_org_mess_name`.execute(db);
}
//# sourceMappingURL=00013_inventory_procurement_performance_indexes.js.map