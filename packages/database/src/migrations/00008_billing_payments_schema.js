...
async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('organization_id', 'uuid', (col) => col.notNull().unique().references('organizations.id').onDelete('cascade'))
        .addColumn('grace_period_days', 'integer', (col) => col.notNull().defaultTo(5))
        .addColumn('late_fee_per_day', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `100.00`))
        .addColumn('default_billing_cycle', 'varchar(30)', (col) => col.notNull().defaultTo('JOINING_DATE'))
    ...
    await db.schema
        ...
        .addColumn('resident_id', 'uuid', (col) => col.notNull().references('residents.id').onDelete('cascade'))
        ...
        .addColumn('subtotal_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('discount_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `0.00`))
        .addColumn('tax_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `0.00`))
        .addColumn('total_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('paid_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `0.00`))
        .addColumn('balance_due_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('status', 'varchar(30)', (col) => col.notNull().defaultTo('ISSUED'))
        .addColumn('issued_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .addColumn('cancelled_at', 'timestamptz')
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .addCheckConstraint('check_invoice_status', (0, kysely_1.sql) `status IN ('DRAFT', 'ISSUED', 'PARTIALLY_PAID', 'PAID', 'OVERDUE', 'CANCELLED')`)
        .addCheckConstraint('check_invoice_amounts', (0, kysely_1.sql) `total_amount >= 0 AND paid_amount >= 0 AND balance_due_amount >= 0`)
        .execute();
    await db.schema
    ...
    await db.schema
        ...
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo((0, kysely_1.sql) `gen_random_uuid()`))
        .addColumn('organization_id', 'uuid', (col) => col.notNull().references('organizations.id').onDelete('cascade'))
        .addColumn('invoice_id', 'uuid', (col) => col.notNull().references('invoices.id').onDelete('cascade'))
        .addColumn('charge_type', 'varchar(50)', (col) => col.notNull())
        .addColumn('description', 'varchar(255)', (col) => col.notNull())
        .addColumn('unit_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('quantity', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull().defaultTo((0, kysely_1.sql) `1.00`))
        .addColumn('total_amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addCheckConstraint('check_invoice_item_charge_type', (0, kysely_1.sql) `charge_type IN ('BASE_RENT', 'FACILITY', 'MESS', 'ADDITIONAL_CHARGE', 'LATE_FEE', 'DISCOUNT', 'ADJUSTMENT')`)
        .execute();
    await db.schema
    ...
    await db.schema
        ...
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .addCheckConstraint('check_payment_method', (0, kysely_1.sql) `payment_method IN ('CASH', 'UPI', 'BANK_TRANSFER', 'CARD', 'OTHER')`)
        .addCheckConstraint('check_payment_status', (0, kysely_1.sql) `status IN ('COMPLETED', 'REVERSED')`)
        .addCheckConstraint('check_payment_amount_pos', (0, kysely_1.sql) `amount > 0`)
        .execute();
    await db.schema
    ...
    await db.schema
        ...
        .addColumn('invoice_id', 'uuid', (col) => col.notNull().references('invoices.id').onDelete('cascade'))
        .addColumn('amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('allocated_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .addCheckConstraint('check_allocation_amount_pos', (0, kysely_1.sql) `amount > 0`)
        .execute();
    // 6. RECEIPTS
    await db.schema
        ...
        .addColumn('id', 'uuid', (col) => col.primaryKey().defaultTo((0, kysely_1.sql) `gen_random_uuid()`))
        .addColumn('organization_id', 'uuid', (col) => col.notNull().references('organizations.id').onDelete('cascade'))
        .addColumn('payment_id', 'uuid', (col) => col.notNull().unique().references('payments.id').onDelete('cascade'))
        .addColumn('receipt_number', 'varchar(50)', (col) => col.notNull())
        .addColumn('resident_id', 'uuid', (col) => col.notNull().references('residents.id').onDelete('cascade'))
        .addColumn('stay_id', 'uuid', (col) => col.notNull().references('stays.id').onDelete('cascade'))
        .addColumn('amount', (0, kysely_1.sql) `numeric(12,2)`, (col) => col.notNull())
        .addColumn('payment_method', 'varchar(30)', (col) => col.notNull())
        .addColumn('generated_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `CURRENT_TIMESTAMP`))
        .execute();
    await db.schema
...
//# sourceMappingURL=00008_billing_payments_schema.js.map