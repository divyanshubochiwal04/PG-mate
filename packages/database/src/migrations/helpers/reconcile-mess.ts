...
export async function reconcileMessSchema(db: Kysely<unknown>): Promise<void> {
  ...
  await db.schema
    ...
    .addColumn('current_stock', sql`numeric(12,3)`, (col) => col.notNull().defaultTo(sql`0.000`))
    .addColumn('minimum_stock', sql`numeric(12,3)`, (col) => col.notNull().defaultTo(sql`0.000`))
    .addColumn('reorder_level', sql`numeric(12,3)`, (col) => col.notNull().defaultTo(sql`0.000`))
    ...
    .addForeignKeyConstraint(
      'fk_inv_items_mess_org',
      ['mess_id', 'organization_id'],
      'messes',
      ['id', 'organization_id'],
      (cb) => cb.onDelete('cascade')
    )
    .addCheckConstraint('check_inv_stock', sql`current_stock >= 0`)
    .addCheckConstraint(
      'check_inv_status',
  ...
  await db.schema
    ...
    .addColumn('quantity', sql`numeric(12,3)`, (col) => col.notNull())
    .addColumn('stock_before', sql`numeric(12,3)`, (col) => col.notNull())
    .addColumn('stock_after', sql`numeric(12,3)`, (col) => col.notNull())
    .addColumn('unit', 'varchar(20)', (col) => col.notNull())
    .addColumn('procurement_id', 'uuid')
    .addColumn('notes', 'varchar(500)')
    .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`NOW()`))
    .addForeignKeyConstraint(
      'fk_inv_trans_item_org',
  ...
  await db.schema
    ...
    .addCheckConstraint('check_proc_total', sql`total_amount >= 0`)
    .execute();
  ...
  await db.schema
    ...
    .addCheckConstraint('check_proc_item_qty', sql`quantity > 0`)
    .addCheckConstraint('check_proc_item_price', sql`unit_price >= 0`)
    .addCheckConstraint('check_proc_item_total', sql`total_price >= 0`)
    .execute();
  ...
  await db.schema
    ...
    .addColumn('updated_at', 'timestamptz', (col) => col.notNull().defaultTo(sql`NOW()`))
    .addForeignKeyConstraint(
      'fk_expenses_mess_org',
      ['mess_id', 'organization_id'],
      'messes',
      ['id', 'organization_id'],
      (cb) => cb.onDelete('restrict')
    )
    .addCheckConstraint('check_exp_amount', sql`amount > 0`)
    .addCheckConstraint(
      'check_exp_category',
