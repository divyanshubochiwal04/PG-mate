...
async function up(db) {
    ...
    await db.schema
        ...
        .addCheckConstraint('chk_tasks_status', (0, kysely_1.sql) `status IN ('TODO', 'IN_PROGRESS', 'COMPLETED', 'CANCELLED')`)
        .addCheckConstraint('chk_tasks_priority', (0, kysely_1.sql) `priority IN ('LOW', 'MEDIUM', 'HIGH', 'CRITICAL')`)
        .execute();
    // 2. Indexes for tasks
    await db.schema
  ...
  `.execute(db);
    ...
    await db.schema
        ...
        .addColumn('note', 'text')
        .addColumn('created_at', 'timestamptz', (col) => col.notNull().defaultTo((0, kysely_1.sql) `now()`))
        .addCheckConstraint('chk_task_activities_action', (0, kysely_1.sql) `action IN ('CREATED', 'UPDATED', 'ASSIGNED', 'UNASSIGNED', 'STARTED', 'COMPLETED', 'CANCELLED', 'REOPENED')`)
        .execute();
...
//# sourceMappingURL=00015_tasks_schema.js.map